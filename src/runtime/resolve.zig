...
pub fn resolve(
    ...
    // 3b. Resolve reasoning_effort
    //     bolt-* aliases encode reasoning_effort; explicit request overrides.
    const reasoning_effort: ?[]const u8 = blk: {
        if (request.reasoning_effort) |re| break :blk re;
        if (request.model) |m| {
            if (aliasReasoningEffort(m)) |are| break :blk are;
        }
        break :blk null;
    };
...
fn expandAlias(m: []const u8) []const u8 {
    ...
    if (std.mem.eql(u8, m, "bolt-light")) return "gpt-5.4";
    if (std.mem.eql(u8, m, "bolt-medium")) return "gpt-5.4";
    if (std.mem.eql(u8, m, "bolt-pro")) return "gpt-5.4";
    if (std.mem.eql(u8, m, "bolt")) return "gpt-5.4";
    if (std.mem.eql(u8, m, "spark")) return "gpt-5.3-codex-spark";
    // Claude aliases
...
/// Returns reasoning_effort implied by bolt-* aliases, or null.
fn aliasReasoningEffort(m: []const u8) ?[]const u8 {
    if (std.mem.eql(u8, m, "bolt-light")) return "low";
    if (std.mem.eql(u8, m, "bolt-medium")) return "medium";
    if (std.mem.eql(u8, m, "bolt-pro")) return "high";
    return null;
}
