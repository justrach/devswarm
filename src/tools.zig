...
pub const tools_list =
    ...
    \\{"name":"run_agent","description":"Run a single Claude agent turn via the Claude Code CLI (`claude -p`). Supports tool allowlists and permission modes. Falls back to codex app-server if `claude` is not on PATH. Set AGENT_SDK_BACKEND=codex to force the legacy backend.","inputSchema":{"type":"object","properties":{"prompt":{"type":"string","description":"The task or question for the agent"},"allowed_tools":{"type":"string","description":"Comma-separated Claude Code tool allowlist, e.g. \"Bash,Read,Edit\". Omit to allow all tools."},"permission_mode":{"type":"string","enum":["default","acceptEdits","bypassPermissions"],"description":"Permission mode for file and shell operations"},"writable":{"type":"boolean","description":"Allow file writes (maps to bypassPermissions when permission_mode is unset)"},"cwd":{"type":"string","description":"Working directory override (default: current repo path)"}},"required":["prompt"]}}
    \\]}
;
