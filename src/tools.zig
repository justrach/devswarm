...
pub const tools_list =
    ...
    \\{"name":"run_swarm","description":"Spawn a self-organizing swarm of parallel agents to tackle a task. An orchestrator decomposes the task into sub-tasks, up to max_agents run concurrently, and a synthesis agent combines their outputs. Set writable=true to allow agents to edit files. Best for broad research, multi-file analysis, multi-angle reviews, or parallel bug fixing.","inputSchema":{"type":"object","properties":{"prompt":{"type":"string","description":"The full task description for the swarm"},"title":{"type":"string","description":"Short human-readable label shown during execution, e.g. \"Implement RCN system\""},"max_agents":{"type":"integer","description":"Maximum parallel sub-agents (default 5, hard cap 100)"},"writable":{"type":"boolean","description":"Allow agents to edit files and run shell commands (default false = read-only analysis)"}},"required":["prompt"]}},
...
fn handleRunSwarm(alloc: std.mem.Allocator, args: *const std.json.ObjectMap, out: *std.ArrayList(u8)) void {
    ...
    const title = mj.getStr(args, "title"); // optional display label
    ...
    swarm.runSwarm(alloc, prompt, title, max_agents, out, writable);
}
