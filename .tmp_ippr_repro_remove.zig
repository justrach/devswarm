...
pub fn main() !void {
    ...
    // simulate removing edge 1->3 by rebuilding graph and calling onEdgeRemoved
    var g2 = graph.CodeGraph.init(alloc);
    defer g2.deinit();
    try g2.addEdge(.{ .src = 1, .dst = 2, .kind = .calls, .weight = 1.0 });

    try ip.onEdgeRemoved(1, 3);
    try ip.deltaUpdate(&g2);

    var full2 = try ppr.pprPush(&g2, 1, ppr.DEFAULT_ALPHA, ppr.DEFAULT_EPSILON, alloc);
    defer full2.deinit();
